    ...
    <div class="row card p-3">
        ...
        <form id="InvoiceForm" method="post">
            <div class="row">
                ...
                <div class="col-lg-4">
                    <label for="" class="p-0 m-0">Invoice Name</label>
                    <?=(isset($edit) ? '<input type="hidden" name="invoice_id" value="' . $edit . '">' : '')?>
                ...
                <div class="col-lg-4">
                    <label for="" class="p-0 m-0">Invoice Header</label>
                    <div class="input-group mb-2">
                        <div class="input-group-prepend">
                            <span class="input-group-text"><i class="fa fa-user"></i></span>
                        </div>
                        <input maxlength="50" autocomplete="off" type="text" id="header" name="header" class="form-control p-0 px-2"
                            placeholder="Image,location" required="" value="<?=(isset($data)) ? $data['header'] : ''?>">
                    </div>
                </div>
...
<script>
    ...
    function submitForm() {
        ...
        var url = '<?=base_url('/config/invoice/saveInvoice')?>';
        $.ajax({
            type: "POST",
            url: url,
            data: fd,
            ...
            success: function (response) {
                ...
                if(response.status){
                    location.href = location.origin + '/config/invoice';
